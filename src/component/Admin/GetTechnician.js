...
export const GetTechnician = () => {
  const [Technican, setTechnicain] = useState([]);
  useEffect(() => {
    axios.get("https://localhost:7093/api/Technician").then((response) => {
      ...
      <Header>
        <div className="fluid mt-5 ms-5 ">
          <div className="">
            ...
            <h3 className="mb-5">Technicians</h3>
            ...
            <table className="table table-striped">
              ...
              <tbody>
                {Technican.map((key, index) => (
                  <tr key={index}>
                    <td>{key.id} </td>
                    <td>{key.fullName} </td>

                    <td>{key.user.role.userRole} </td>
                    <td>{key.technicianLocation}</td>
